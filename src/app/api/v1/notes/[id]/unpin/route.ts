/**
 * Next.js API Route: /api/v1/notes/[id]/unpin
 *
 *   - POST → unpin a note (clears isPinned). Returns the updated note.
 *
 * A write, so it mirrors the other mutating routes: per-IP rate limit first,
 * then API-key auth. Unpinning a missing note yields 404 NOTE_NOT_FOUND and an
...
export async function POST(request: NextRequest, { params }: RouteContext) {
  ...
  const result = await NoteController.unpinNote(params.id);
